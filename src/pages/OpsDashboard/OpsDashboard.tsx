...
const OpsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const bidding = orderData.filter(order => order.order_status === 'bidding').length;
    const approved = orderData.filter(order => order.order_status === 'approved').length;
    const completed = orderData.filter(order => order.order_status === 'completed').length;
    const totalPaid = orderData.filter(order => order.order_paid === 'paid').reduce((sum, order) => sum + order.order_amount, 0);
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats title="Bidding Orders" total={`${biddingCount}`} rate="2%" levelUp>
          <svg
        ...
        <CardDataStats title="Orders in Progress" total={`${approvedCount}`} rate="40%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="20"
            height="22"
        ...
        <CardDataStats title="Completed Orders" total={`${completedCount}`} rate="2.59%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="22"
        ...
        <CardDataStats title="Amount Spent" total={`$${totalPaidAmount}`} rate="22.5%" levelUp>
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
